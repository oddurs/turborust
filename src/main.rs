...
fn cmd_init() -> Result<i32> {
    ...
    let meta = cargo::Metadata::load(&root).unwrap_or(None);
    let mut out =
        String::from("# turborust.toml — see `turborust plan` for the resolved graph\n\n");

    match &meta {
        Some(m) if !m.packages.is_empty() => {
            let bins: Vec<&cargo::Package> = m.packages.values().filter(|p| p.has_bin).collect();
            let wasm: Vec<&cargo::Package> =
                m.packages.values().filter(|p| p.is_wasm_lib).collect();
            for p in &bins {
                out.push_str(&format!(
                    "[services.{}]\n\
                     # cmd, watch globs and rebuild edges are derived from cargo —\n\
                     # including this crate's path dependencies.\n\
                     cargo = \"{}\"\n\
                     # port = 8080\n\n",
                    p.name, p.name
                ));
        ...
        _ => out.push_str(EXAMPLE),
    }

    std::fs::write(&path, out)?;
    println!("wrote {}", path.display());
    println!("next: `turborust plan`, then `turborust up`");
    Ok(0)
}
